...
import { packCAR, uploadCARWithUCAN, uploadCARWithApiToken } from '../utils/ipfs.js'
import type { Config } from '../types.js'



export const deployToIpfs = async (folder: string, {provider}: Config, ci = false) => {
  ...
  const response = ci ? await uploadCARWithApiToken(carFile, provider) : await uploadCARWithUCAN(carFile, provider)
